 ...
 * overflow CTC mode
 */
#define TIM0_CTC();					TCCR0A |= (1<<WGM01);TCCR0A &= ~(1<<WGM00);
#define TIM0_compare_A()		TIMSK0 |= (1<<OCIE0A);
 ...
 * Overflow CTC
 */
#define TIM1_CTC();				TCCR1B |= (1<<WGM12); TCCR1A &= ~(1<<WGM11)|~(1<<WGM10); TCCR1B &= ~(1<<WGM13);
#define TIM1_overflow_COMPA();	TIMSK1 |= (1<<OCIE1A);	
 ...
 * overflow CTC mode
 */
#define TIM2_CTC();					TCCR2A |= (1<<WGM21);TCCR2A &= ~(1<<WGM20);TCCR2B &= ~(1<<WGM22);
#define TIM2_overflow_COMPA()		TIMSK2 |= (1<<OCIE2A);
